# FamilyOS — Claude Code Engineering Instructions

## Purpose

This file defines the repository-level engineering instructions for Claude Code when working on FamilyOS.
...
Claude Code is an engineering assistant operating within this repository. Its role is to help preserve and improve that coherence while respecting the authority, architecture, contracts, and human-controlled workflow of FamilyOS.
...
Read-only Git inspection is allowed when relevant and permitted by the active Claude Code permission mode.
...
Do not assume uncommitted changes were created by Claude.
...
FamilyOS may be developed with multiple AI engineering assistants, including Claude Code and ChatGPT.
...
Claude Code must not independently expand the scope of work.
