...
from familyos_cli.plugins.ecosystem.resolution import PluginVersion

VALIDATOR_ID = "metadata.version-format"


class MetadataVersionFormatValidator(ComplianceValidator):
    """Validate that the manifest version is a valid semantic version."""

    def validate(
        self,
        context: ValidationContext,
    ) -> ValidatorRunResult:
        """Collect evidence of manifest version validity."""
        ...
        raw_version = context.manifest.get("version") or ""

        try:
            PluginVersion.parse(raw_version)
            valid = True
        except ValueError:
            valid = False
    ...
    def check(
        ...
        """Return the outcome of the version-format check."""
