...
class SpecificationValidator:
    ...
    def validate(self, specification: dict[str, Any]) -> None:
        ...
        if "directories" not in project:
            raise ValidationError("Missing required field: 'project.directories'.")

        if "files" not in project:
            raise ValidationError("Missing required field: 'project.files'.")

        if not isinstance(project["directories"], list):
            raise ValidationError("'project.directories' must be a list.")
        ...
        for index, file in enumerate(project["files"]):
            if "destination" not in file:
                raise ValidationError(f"Missing 'destination' in file #{index}.")

            if "template" not in file:
                raise ValidationError(f"Missing 'template' in file #{index}.")
